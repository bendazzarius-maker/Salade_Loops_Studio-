...
(function initAudioBackendGlobal() {
  ...
  function getDrumUiState(params) {
    if (!params || typeof params !== "object") return null;
    const direct = params.__drumMachineUiState || params.drumMachineUiState || params.drumKitState || params.drumkitState || null;
    if (direct && typeof direct === "object") return direct;
    return null;
  }

  function getDrumRowMixChannel(params, note, fallback = 1) {
    const uiState = getDrumUiState(params);
    const rows = uiState?.project?.mappingRows;
    if (!Array.isArray(rows)) return Math.max(1, Math.floor(Number(fallback || 1)));
    const midi = Math.floor(Number(note || 0));
    const row = rows.find((r) => Math.floor(Number(r?.midi ?? r?.midiNote ?? -1)) === midi);
    const ch = Math.floor(Number(row?.channel || fallback || 1));
  ...
  class AudioBackendJUCE {
    ...
    async _request(op, data = {}) {
      ...
      const req = this._buildReq(op, data);
      ...
      if (
        ...
        op === "touski.note.off" ||
        op === "mixer.master.set" ||
        op === "mixer.channel.set" ||
        op === "fx.chain.set"
      ) {
        console.log(`[JUCE][REQ] ${op}`, {
          id: req.id,
          data,
          req
        });
      ...
      const res = await window.audioNative.request(req);
      ...
      if (!res?.ok) {
        console.error(`[JUCE][REQ ERROR] ${op}`, {
          id: req.id,
          data,
          err: res?.err || null,
          res
    ...
    _evt(msg) {
      ...
      if (msg.op === "transport.state") {
        ...
        try {
          window.dispatchEvent(new CustomEvent("sls:transport-state", { detail: { ...this.transportState } }));
        } catch (_) {}
    ...
    async setTransportRange(payload = {}) { return this._request("transport.range.set", payload || {}); }
    ...
    async previewNoteOn({ note, velocity = 0.85, trackId = "preview", instId = "global", instType = "piano", params = {}, mixCh = 1 }) {
      const lowerType = String(instType || "").toLowerCase();
      if (lowerType.includes("touski") || lowerType.includes("sample touski")) {
        const safeInstId = String(instId || "touski");
        const safeMixCh = Math.max(1, Math.floor(Number(mixCh || 1)));
        const safeParams = (params && typeof params === "object") ? params : {};
        const programPath = normalizePath(safeParams?.programPath || safeParams?.path || safeParams?.file || "");
        if (programPath) {
          console.log("[PREVIEW][TOUSKI][LOAD]", {
            instId: safeInstId,
            instType,
            note,
            velocity,
          ...
          const loadRes = await this.touskiProgramLoad({ instId: safeInstId, programPath });
          if (!loadRes?.ok) return loadRes;
        }

        console.log("[PREVIEW][TOUSKI][PARAM.SET]", {
          instId: safeInstId,
          instType,
          params: safeParams
        });
        const paramRes = await this._request("touski.param.set", {
          instId: safeInstId,
          programPath,
          params: safeParams
        });
        if (!paramRes?.ok) return paramRes;

        console.log("[PREVIEW][TOUSKI][NOTE.ON]", {
          instId: safeInstId,
          instType,
          note,
          velocity,
        ...
        return this.touskiNoteOn({ instId: safeInstId, note, velocity, mixCh: safeMixCh });
      }
      ...
      const safeParams = (params && typeof params === "object") ? params : {};
      const resolvedMixCh = lowerType.includes("drum") ? getDrumRowMixChannel(params, note, mixCh) : mixCh;
      const safeMixCh = Number(resolvedMixCh || 1);

      const juceSpec = window.JuceInstructionLibrary?.buildInstrumentSpec?.({
        name: safeType,
        params: safeParams,
        instId: safeInstId,
        trackId
      ...
      if (!this._instCreated.has(safeInstId)) {
        console.log("[PREVIEW][INST.CREATE]", {
          instId: safeInstId,
          type: safeType,
          ch: 0
        });
      ...
      console.log("[PREVIEW][PARAM.KEY]", {
        instId: safeInstId,
        previous: this._instParamHash.get(safeInstId) || null,
        next: paramKey,
        changed: this._instParamHash.get(safeInstId) !== paramKey
      });

      if (this._instParamHash.get(safeInstId) !== paramKey) {
        console.log("[PREVIEW][INST.PARAM.SET]", {
          instId: safeInstId,
          type: safeType,
          params: safeParams,
          juceSpec
    ...
    async previewNoteOff({ note, trackId = "preview", instId = "global", instType = "piano", params = {}, mixCh = 1 }) {
      const lowerType = String(instType || "").toLowerCase();
      if (lowerType.includes("touski") || lowerType.includes("sample touski")) {
        const safeInstId = String(instId || "touski");
        const safeMixCh = Math.max(1, Math.floor(Number(mixCh || 1)));
        console.log("[PREVIEW][TOUSKI][NOTE.OFF]", {
          instId: safeInstId,
          instType,
          note,
          mixCh: safeMixCh,
        ...
        return this.touskiNoteOff({ instId: safeInstId, note, mixCh: safeMixCh });
      }
    ...
    async triggerNote({ note, velocity = 0.85, trackId = "preview", durationSec = 0.25, instId = "global", instType = "piano", params = {}, mixCh = 1 }) {
      const startReq = await this.previewNoteOn({ note, velocity, trackId, instId, instType, params, mixCh });
      if (!startReq?.ok) return startReq;

      setTimeout(() => {
        this.previewNoteOff({ note, trackId, instId, instType, params, mixCh }).catch(() => {});
  ...
  class AudioBackendController {
    ...
    async previewNoteOn(payload) {
      if (this.active !== "juce") return;
      const res = await this.backends.juce.previewNoteOn(payload);
      if (!res?.ok) return;
      return res;
    }

    async previewNoteOff(payload) {
      if (this.active !== "juce") return;
      const res = await this.backends.juce.previewNoteOff(payload);
      if (!res?.ok) return;
      return res;
    }
