...
function _normalizeLfoTargetRef(ref){
  ...
  if(out.kind === "fx"){
    if(!String(out.param||"").trim()) out.param = "wet";
  }
...
function updateLfoInspector(){
  ...
  if(!wantFx){
    ...
    bind.fxIndex = Number(fxSel.value||0);

    const paramsObj = fxArr[bind.fxIndex]?.params || {};
    const keys = Object.keys(paramsObj);
    if(keys.length===0){
      const op=document.createElement("option");
      op.value=(bind.param||"wet");
      op.textContent="(param FX — à câbler)";
      paramSel.appendChild(op);
      _safeSetSelectValue(paramSel, bind.param || "wet", bind.param || "wet");
    }else{
      for(const k of keys){
        const op=document.createElement("option");
        op.value=k;
